...
def quality_focal_loss_with_prob(pred, target, beta=2.0):
    ...
    with torch.cuda.amp.autocast(enabled=False):
        loss = F.binary_cross_entropy(
            pred.float(), zerolabel.float(), reduction='none'
        ) * scale_factor.float().pow(beta)
        ...
        if pos.numel() > 0:
            ...
            pos_loss = F.binary_cross_entropy(
                pred[pos, pos_label].float(), score[pos].float(), reduction='none'
            ) * scale_factor.abs().float().pow(beta)
