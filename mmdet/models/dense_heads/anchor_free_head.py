...
class AnchorFreeHead(BaseDenseHead):
    ...
    Args:
        ...
        stacked_convs (int): Number of stacking convs of the head.分类卷积曾的数量
    ...
    def _init_cls_convs(self) -> None:
        ...
        for i in range(self.stacked_convs):  #四层
            chn = self.in_channels if i == 0 else self.feat_channels
            if self.dcn_on_last_conv and i == self.stacked_convs - 1:  #表示是否在最后一个卷积层使用可变形卷积（DCNv2）
                conv_cfg = dict(type='DCNv2')
            else:
                conv_cfg = self.conv_cfg
            self.cls_convs.append(
                ConvModule(  #一个封装了卷积层、归一化层和激活层的模块。
                    chn,
                    self.feat_channels,
                    3,
                    stride=1,
    ...
    def _init_predictor(self) -> None:  #分类和回归头
