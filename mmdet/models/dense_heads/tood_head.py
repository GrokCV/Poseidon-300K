...
class TaskDecomposition(nn.Module):
    ...
    def forward(self,
        ...
        weight = self.layer_attention(avg_feat)   #学习得到一种适配与当前任务的权重系数w_k
        ...
        conv_weight = weight.reshape(
            ...
            1) * self.reduction_conv.conv.weight.reshape(
                1, self.feat_channels, self.stacked_convs, self.feat_channels)  #将层注意力权重与卷积权重相乘，生成新的卷积权重。
        conv_weight = conv_weight.reshape(b, self.feat_channels,
                                          self.in_channels)
        feat = feat.reshape(b, self.in_channels, h * w)
        feat = torch.bmm(conv_weight, feat).reshape(b, self.feat_channels, h,
                                                    w)   ##得到w_k*X_inter，即X_task
