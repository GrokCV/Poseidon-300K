...
class MMCVMSDeformAttn(MultiScaleDeformableAttention):
...
class Extractor(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, n_levels=1, deform_ratio=1.0,
        ...
        if MMCV_ATTENTION:
            self.attn = MMCVMSDeformAttn(embed_dims=dim, num_levels=n_levels, num_heads=num_heads,
    ...
    def forward(self, query, reference_points, feat, spatial_shapes, level_start_index, H, W):
        ...
        def _inner_forward(query, feat):
            if MMCV_ATTENTION:
                attn = self.attn(self.query_norm(query), None, self.feat_norm(feat), None, None, None,
        ...
        if self.with_cp and query.requires_grad and not MMCV_ATTENTION:
...
class Injector(nn.Module):
    ...
    def forward(self, query, reference_points, feat, spatial_shapes, level_start_index):
        ...
        def _inner_forward(query, feat):
            if MMCV_ATTENTION:
                attn = self.attn(self.query_norm(query), None, self.feat_norm(feat), 0, None, None,
                                reference_points, spatial_shapes,level_start_index)
...
class SpatialPriorModule(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, out_indices=(0, 1, 2, 3),with_cp=False):
        super().__init__()
        self.out_indices = out_indices  #输入通道数，默认为 64
        self.embed_dim = embed_dim   #嵌入维度，最终特征图将被转换为此维度，默认为 384
        self.with_cp = with_cp
    ...
    def forward(self, x):
        def _inner_forward(x):
            ...
            c2 = self.fc2(c2)
            c3 = self.fc3(c3)
            c4 = self.fc4(c4)  #每一层的特征图都通过一个 1x1 卷积映射到 embed_dim
            return c1,c2,c3,c4
        if self.with_cp and x.requires_grad and not MMCV_ATTENTION:
