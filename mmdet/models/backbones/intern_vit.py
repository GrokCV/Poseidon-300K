...
class InternVisionEmbeddings(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, flatten=True):
        ...
        self.num_positions = self.num_patches + 1   #表示模型中总共有多少个位置，比如14*14=196
        #为每个 patch 提供位置信息
    ...
    def forward_adapter(self, x):
        ...
        _, _, H, W = patch_embeds.shape  #获取每个patch的高度和宽度
...
class InternViT(BaseModule):
    ...
    def init_weights(self, pretrained=None,pretrained_type='ViT'):
        ...
        if isinstance(pretrained, str):
            #logger = logger
            logger.info('load pretrained weights from %s, type=%s', pretrained, pretrained_type)
