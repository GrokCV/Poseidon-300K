...
class DetDataPreprocessor(ImgDataPreprocessor):
    ...
    1. 支持批次级别的数据增强
    2. 将这些信息batch_input_shape and pad_shape添加到 data_samples（数据样本）中
  

    It provides the data pre-processing as follows

    - 收集移动数据到目标设备BaseDataPreprocessor.cast_data
    - 将输入填充到当前批次的最大尺寸，并使用定义的 pad_value
     填充后的尺寸可以被定义的 pad_size_divisor 整除。
    - 将输入堆叠成 batch_inputs
     一旦所有图像都调整到相同尺寸，它们就会被堆叠起来，形成一个包含多张图像的单一大张量，这就是模型的批次输入。
    - 如果输入形状是 (3, H, W)，则将输入从 BGR 转换为 RGB。
    - 使用定义的 std 和 mean 对图像进行归一化。
    - 在训练期间进行批次增强。
    Args:
        mean (Sequence[Number], optional): The pixel mean of R, G, B channels.
            Defaults to None.用于图像归一化的均值
        std (Sequence[Number], optional): The pixel standard deviation of
            R, G, B channels. Defaults to None.
        pad_size_divisor (int): 填充后图像尺寸必须是它的倍数，默认为 1
        pad_value (Number): 图像填充时使用的像素值，默认为 0。
        pad_mask (bool): 是否填充实例掩码，默认为 False。
        mask_pad_value (int): 实例掩码填充时使用的像素值，默认为 0。
        pad_seg (bool): 是否填充语义分割图，默认为 False。
        seg_pad_value (int): 语义分割图填充时使用的像素值，默认为 255。
        bgr_to_rgb (bool): 是否将图像从 BGR 转换为 RGB，默认为 False。
        rgb_to_bgr (bool): 是否将图像从 RGB 转换为 BGR，默认为 False。
        boxtype2tensor (bool): 是否将边界框数据从 BaseBoxes 类型转换为 Tensor 类型，默认为 True。
        non_blocking (bool): 在数据传输到设备（如 GPU）时是否阻塞当前进程，默认为 False。
        batch_augments (list[dict], optional): 一个字典列表，定义了要在批次级别应用的增强操作。
    """
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        data = super().forward(data=data, training=training)  #调用父类方法，所以时以及pad后的结果
        inputs, data_samples = data['inputs'], data['data_samples']  

        if data_samples is not None:
            #将批量输入形状和填充形状设置到每个数据样本的元信息
            # NOTE the batched image size information may be useful, e.g.
            # in DETR, this is needed for the construction of masks, which is
            # then used for the transformer_head.inputs：每个张量的形状为 (N,C,H,W)
            batch_input_shape = tuple(inputs[0].size()[-2:])  #高度和宽度转换为元组，表示批量输入的形状。
            for data_sample, pad_shape in zip(data_samples, batch_pad_shape):
                data_sample.set_metainfo({   #用于设置数据样本的元信息。
                    'batch_input_shape': batch_input_shape,
                    'pad_shape': pad_shape
                })
#对data_samples进行填充这种操作
...
#遍历 self.batch_augments 中的每个批量增强模块 batch_aug。对输入数据 inputs 和数据样本 data_samples 应用批量增强模块 batch_aug
    ...
    def _get_pad_shape(self, data: dict) -> List[tuple]:
        ...
        pad_size_divisor.主要就是为了保证data['inputs']的高宽在填充后是pad_size_divisor的倍数
        最后返回【（高宽），（）】"""
        _batch_inputs = data['inputs']   #提取 inputs 键对应的值，即图像张量或图像张量列表。
        # Process data with `pseudo_collate`.
        if is_seq_of(_batch_inputs, torch.Tensor):  #如果是一个张量列表
            batch_pad_shape = []
            for ori_input in _batch_inputs:
                pad_h = int(  #计算填充高度，使得填充后高度是pad_size_divisor的倍数
                    np.ceil(ori_input.shape[1] /
                            self.pad_size_divisor)) * self.pad_size_divisor
                pad_w = int(
                    np.ceil(ori_input.shape[2] /
                            self.pad_size_divisor)) * self.pad_size_divisor
                batch_pad_shape.append((pad_h, pad_w))  #存放（（高，宽））
        # Process data with `default_collate`.
        elif isinstance(_batch_inputs, torch.Tensor):
            assert _batch_inputs.dim() == 4, (  #确保维度是（nchw）
    ...
    def pad_gt_masks(self,
        ...
        if 'masks' in batch_data_samples[0].gt_instances: #检查第一个数据样本的 gt_instances 是否包含 masks 属性
    ...
    def pad_gt_sem_seg(self,
        ...
        if 'gt_sem_seg' in batch_data_samples[0]:
            for data_samples in batch_data_samples:
                gt_sem_seg = data_samples.gt_sem_seg.sem_seg  #获取语义分割掩码。
                h, w = gt_sem_seg.shape[-2:]  #获取掩码宽高
                ...
                data_samples.gt_sem_seg = PixelData(sem_seg=gt_sem_seg)  #更新
